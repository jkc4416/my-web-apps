...
FunAppBox — a Next.js 15 monorepo containing 14 standalone mini web apps and games served under a single domain (www.funappbox.com). Each app is a client component (`"use client"`) with its own route and SEO layout.
...
- **State:** All client-side, no backend/database. D-Day, hamster, snake, flappy, fortune apps use localStorage.
...
- **SEO:** Each app has its own `layout.jsx` exporting `metadata` for title/description/keywords/OG/Twitter. Hub page has JSON-LD structured data.
- **Sitemap:** Auto-generated via `src/app/sitemap.js` with priority differentiation
...
- Game apps (snake, flappy, hamster) and fortune app were written directly as route pages
...
- All 14 apps are client components using React hooks — `"use client"` directive is required at top of each page.jsx
- All localStorage calls are wrapped in try/catch for Safari private browsing compatibility
- All navigator.clipboard calls are wrapped in try/catch for non-HTTPS fallback
- The invest app uses Recharts for charting, data spans 2005~2026 with period presets
- The sound app uses Web Audio API (AudioContext)
- Snake and flappy games use Canvas API with mobile-responsive scaling
- Hamster game uses CSS pixel art with 5-tier growth sprites, timer refs for leak prevention
